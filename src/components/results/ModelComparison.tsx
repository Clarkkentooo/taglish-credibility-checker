    ...
    <Card className="p-5">
      <button type="button" className="flex w-full min-h-11 items-center justify-between text-left" onClick={() => setOpen((value) => !value)} aria-expanded={open}>
        <span>
          ...
          <span className="block text-sm text-muted">Model agreement improves consistency, but does not prove factual truth.</span>
        </span>
          ...
          {scores.map((score) => (
            <div key={score.model}>
              <div className="mb-1 flex justify-between text-sm">
                ...
                <span className="text-muted">{Math.round(score.credibleProbability * 100)}% credible</span>
              </div>
